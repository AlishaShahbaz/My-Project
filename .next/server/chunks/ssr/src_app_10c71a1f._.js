...
;
function SignupForm() {
    const { register, handleSubmit, watch, setValue, resetField, formState: { errors } } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$react$2d$hook$2d$form$2f$dist$2f$index$2e$esm$2e$mjs__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useForm"])({
        defaultValues: {
            countryCode: 'US'
        }
    });
    ...
    const onSubmit = async (data)=>{
        ...
        try {
            ...
            const result = await res.json();
            if (!res.ok) {
                const serverMsg = result?.error || Array.isArray(result?.issues) && result.issues.map((i)=>`${i.field}: ${i.message}`).join(' | ') || 'Signup failed';
...
//# sourceMappingURL=src_app_10c71a1f._.js.map